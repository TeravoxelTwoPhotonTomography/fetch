...
#define CHK(expr) \
  ...
  */
  ...
  unsigned int Cut::run(device::Microscope* dc)
  {
    ...
    bz = cz-dz+thick;
    CHK( dc->stage()->setPos(cx,cy,10));           // Drop to safe z first
    CHK( dc->stage()->setPos(ax,ay,10));           // Move on safe z plane
    ...
    CHK( dc->stage()->setPos(bx,by,10));  // Drop to safe z first, come up at an angle -- Note: vibratome still running!
                                  //(0 to 12)   
    ...
    // Move back
    CHK( dc->stage()->setPos(cx,cy,10));           // Move on safe z plane
