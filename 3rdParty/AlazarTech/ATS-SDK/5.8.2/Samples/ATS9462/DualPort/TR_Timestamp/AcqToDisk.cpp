...
BOOL ConfigureBoard(HANDLE boardHandle)
{
	...
	retCode = 
		AlazarSetCaptureClock(
			...
			SAMPLE_RATE_1MSPS,		// U32 -- sample rate id
...
BOOL AcquireData(HANDLE boardHandle)
{
	...
	// Get the number of samples clock periods per timestmap count 
	// used to convert trigger timestamps to seconds
	...
	if (enableHeaders)
	{
		retCode = 
			AlazarGetParameter(
				boardHandle,						// HANDLE -- board handle
				0,									// U8 -- channel 
				GET_SAMPLES_PER_TIMESTAMP_CLOCK,	// U32 -- parameter
				&samplesPerTimestampCount			// long* -- value
				);
		if (retCode != ApiSuccess)
		{
	...
	if (success)
	{
		...
		if (enableHeaders)
			admaFlags |= ADMA_ENABLE_RECORD_HEADERS;	// Precede each record in buffer with 16-byte record header
				...
				for (U32 record = 0; (record < recordsPerBuffer) && (success == TRUE); record++)
				{
					for (int channel = 0; (channel < channelCount) && (success == TRUE); channel++)
					{
						...
						if (enableHeaders)
						{
							...
							timestamp_counts |= (((INT64) (pHeader->hdr3.TimeStampHighPart & 0x0ff)) << 32);

							triggerTimestamp_sec = samplesPerTimestampCount * timestamp_counts / SamplesPerSec;

							// Seek to record samples in buffer

							pRecord += sizeof(ALAZAR_HEADER) / bytesPerSample;
						}
						...
						if (saveData)
						{
							...
							// A 16-bit sample code is stored in each 16-bit sample value in the buffer.

							// Write the sample time in seconds and sample code to file.
							// If record headers are enabled, then the time offset is relative to 
							// trigger event at the start of the acquisition. If not, the time 
							// offset is relative to the the trigger event in the this record.

							double secPerSample = 1. / SamplesPerSec;
							double sampleTimestamp_sec = triggerTimestamp_sec - preTriggerSamples * secPerSample;

							if (fprintf(fpData, "--> Record %u Ch %c begin\n\n", record + 1, channelLetter) < 0)
							{
								printf("Error: Write record %u channel %d header failed -- %u\n", record, channel, GetLastError());
								success = FALSE;
							}
							...
							{
								U16 sampleCode = *pSample++;

								if (fprintf(fpData, "%.9lf, %u\n", sampleTimestamp_sec, sampleCode) < 0)
								{
									printf("Error: Write record %u channel %d sample %u failed -- %u\n", record, channel, sample, GetLastError());
									success = FALSE;
								}
								sampleTimestamp_sec += secPerSample;
							}
							...
							if (success)
							{
								if (fprintf(fpData, "\n<-- Record %u Ch %c end\n\n", record + 1, channelLetter) < 0)
								{
									printf("Error: Write record %u channel %c footer failed -- %u\n", record, channelLetter, GetLastError());
									success = FALSE;
								}
							}
						}
