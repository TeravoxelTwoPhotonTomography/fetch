...
// AcqToDisk.cpp :
//
// This program demonstrates how to configure an ATS660 to capture to on-board 
	...
	for (record = 0; (record < recordsPerCapture) && (success == TRUE); record++)
	{
		for (int channel = 0; (channel < channelCount) && (success == TRUE); channel++)
		{
			...
			else
			{
				...
				// 
				// 16-bit sample codes occupy each 16-bit sample value. 
				//
				...
				if (saveData)
				{
					...
					{
						U16 minCode = *pMinMaxSamples;
						pMinMaxSamples++;

						U16 maxCode = *pMinMaxSamples;
						pMinMaxSamples++;
