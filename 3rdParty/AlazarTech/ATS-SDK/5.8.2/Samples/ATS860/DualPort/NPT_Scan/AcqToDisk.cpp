...
BOOL ConfigureBoard(HANDLE boardHandle)
{
	...
	// Configure the AUX I/O connector as an AutoDMA trigger enable input

	retCode = 
		AlazarConfigureAuxIO (
			boardHandle,			// HANDLE -- board handle
			AUX_IN_TRIGGER_ENABLE,	// U32 -- mode 
			TRIGGER_SLOPE_POSITIVE	// U32 -- parameter (edge of slope)
			);
	if (retCode != ApiSuccess)
	{
...
BOOL AcquireData(HANDLE boardHandle)
{
	...
	// The board will be armed to acquire an infinite number of buffers,
	// but this program will abort the acquistion after this interval.

	double acquistionTime_sec = 10.;
	...
	if (success)
	{
		printf("Capturing ... press any key to abort\n");
		...
		for (;;)
		{
			...
			{
				printf("Acquistion stopped in %lf sec.\n", acquistionTime_sec);
				break;
			}
