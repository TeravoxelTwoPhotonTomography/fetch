...
BOOL AcquireData(HANDLE boardHandle)
{
	...
	U32 admaFlags = ADMA_EXTERNAL_STARTCAPTURE | // Start acquisition when we call AlazarStartCapture 
					...
					ADMA_ALLOC_BUFFERS;			 // API allocates and manages DMA buffers; it will copy data into our buffer
	...
	// Create a binary data file, and enable the API to write AutoDMA buffers to this file.
	...
	if (success)
	{
		...
		while (buffersCompleted < buffersPerAcquisition)
		{
			...
			if (success)
			{
				...
				// contiguously in the buffer, with a 8-bit sample code in 
				// each 8-bit sample value.
				//
