...
U8* BufferArray[MAX_BOARDS][BUFFER_COUNT] = { NULL };
...
BOOL AcquireData(U32 boardCount)
{
	...
	for (boardIndex = 0; (boardIndex < boardCount) && success; boardIndex++)
	{
		for (bufferIndex = 0; (bufferIndex < BUFFER_COUNT) && success; bufferIndex++)
		{
			...
			BufferArray[boardIndex][bufferIndex] = (U8*) VirtualAlloc(NULL, bytesPerBuffer, MEM_COMMIT, PAGE_READWRITE);
#else
			BufferArray[boardIndex][bufferIndex] = (U8*) malloc(bytesPerBuffer);
#endif
	...
	for (boardIndex = 0; (boardIndex < boardCount) && success; boardIndex++)
	{
		...
		// Add the buffers to a list of buffers available to be filled by this board

		for (bufferIndex = 0; (bufferIndex < BUFFER_COUNT) && success; bufferIndex++)
		{
			U8* pBuffer = BufferArray[boardIndex][bufferIndex];
