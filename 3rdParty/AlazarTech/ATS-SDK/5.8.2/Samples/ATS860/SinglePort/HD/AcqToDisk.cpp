...
// AcqToDisk.cpp :
//
// This program demonstrates how to configure an ATS860 to capture to on-board 
...
BOOL AcquireData(HANDLE boardHandle)
{
	...
	U32 preTriggerSamples = 2 * 1024 * 1024;

	// TODO: Select the number of post-trigger samples per record 

	U32 postTriggerSamples = 2 * 1024 * 1024;
	...
	U32 outBytesPerSample = 2;		// 16-bits per HyperDisp sample value
	U32 outSamplesPerPoint = 2;		// 1 for min sample + 1 for max sample
	U32 bytesPerBuffer = outBytesPerSample * outSamplesPerPoint * pointsPerRecord;
	...
	for (record = 0; (record < recordsPerCapture) && (success == TRUE); record++)
	{
		for (int channel = 0; (channel < channelCount) && (success == TRUE); channel++)
		{
			...
			else
			{
				...
				// 
				// 8-bit sample codes occupy the most significant bits
				// of each 16-bit sample value. 
				//
				...
				if (saveData)
				{
					...
					U8 *pMinMaxSamples = (U8*) buffer;
					for (U32 point = 0; (point < pointsPerRecord) && (success == TRUE); point++)
					{
						U8 minCode = *pMinMaxSamples;
						pMinMaxSamples++;

						U8 maxCode = *pMinMaxSamples;
						pMinMaxSamples++;

						U8 aveCode = (U8) (((U32) minCode + (U32) maxCode) / 2);
