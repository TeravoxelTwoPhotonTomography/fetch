...
// AcqToDisk.cpp :
//
// This program demonstrates how to configure an ATS460 to capture to on-board 
...
BOOL AcquireData(HANDLE boardHandle)
{
	...
	// Calculate the amount to shift a sample value to get the 
	// sample code in its most significatn bits.
	...
	for (record = 0; (record < recordsPerCapture) && (success == TRUE); record++)
	{
		for (int channel = 0; (channel < channelCount) && (success == TRUE); channel++)
		{
			...
			else
			{
				...
				// 14-bit sample codes in the most significant bits of each 16-bit 
				// sample value. 
				//
				...
				if (saveData)
				{
					...
					for (U32 sample = 0; (sample < samplesPerRecord) && success; sample++)
					{
						...
						// Get 14-bit sample code from most significant bits of
						// 16-bit sample value
