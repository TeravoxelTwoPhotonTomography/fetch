...
BOOL AcquireData(HANDLE boardHandle)
{
	...
	if (success)
	{
		U32 admaFlags = ADMA_EXTERNAL_STARTCAPTURE |	// Start acquisition when AlazarStartCapture is called
						ADMA_TRIGGERED_STREAMING |		// Acquire a continuous stream of sample data after a trigger event
						ADMA_INTERLEAVE_SAMPLES;		// Interleave samples for higher transfer rates
	...
	if (success)
	{
		...
		while (buffersCompleted < buffersPerAcquisition)
		{
			...
			if (success)
			{
				...
				// 
				// Records in the buffer are arranged as follows: R0[AB]
				// where Rx is a segment of a single record.
				//
				// Sample values from each enabled channel are interleaved in 
				// the buffer (ie. A0B0A1B1...), where a 12-bit sample code 
				// occupies the most significant bits of each 16-bit sample value.
				//
