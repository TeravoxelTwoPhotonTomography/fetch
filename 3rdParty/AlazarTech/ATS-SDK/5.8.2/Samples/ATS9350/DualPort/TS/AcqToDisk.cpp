	...
	if (success)
	{
		...
		while (buffersCompleted < buffersPerAcquisition)
		{
			...
			if (success)
			{
				...
				// where Rxy is a segment of a single continuous record.
				//
				// Samples are arranged contiguously for each channel.
				// A 12-bit sample code occupies the most significant bits of each 
				// 16-bit value in the record.
				//
