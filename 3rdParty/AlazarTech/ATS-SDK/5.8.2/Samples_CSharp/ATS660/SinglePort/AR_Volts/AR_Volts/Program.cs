...
namespace AR_Volts
{
    ...
    class AcqToDiskApp
    {
        ...
        static public bool ConfigureBoard(IntPtr boardHandle)
        {
            ...
            inputRangeIdChA = AlazarAPI.INPUT_RANGE_PM_800_MV;
        ...
        static public unsafe bool AcquireData(IntPtr boardHandle)
        {
            ...
            // Calculate the scale and offset factors required to convert sample values to volts.
            //
            // Given:
            //   sampleVolts = inputRangeVolts * ((sampleValue / valueToCode) - codeZero) / codeRange;
            //
            // Multiply top and bottom by codeToValue and rearrange:
            //   sampleVolts = (inputRangeVolts  / (valueToCode *  codeRange)) * (sampleValue - codeZero * valueToCode);
            //
            // Define:
            //   scaleFactor =  inputRangeVolts / (valueToCode *  codeRange);
            //   offsetFactor = codeZero * codeToValue;

            double inputRangeChA_volts;
            if (!InputRangeIdToVolts(inputRangeIdChA, out inputRangeChA_volts))
            {
                Console.WriteLine("Error: Invalid CHA input range -- " + inputRangeIdChA);
                return false;
            }

            double inputRangeChB_volts;
            if (!InputRangeIdToVolts(inputRangeIdChB, out inputRangeChB_volts))
            {
                Console.WriteLine("Error: Invalid CHB input range -- " + inputRangeIdChB);
                return false;
            }

            int sampleBitShift = (int)(8 * bytesPerSample - bitsPerSample);
            int valueToCode = 1 << sampleBitShift;

            double codeZero = (1 << (bitsPerSample - 1)) - 0.5;
            double codeRange = (1 << (bitsPerSample - 1)) - 0.5;

            double offsetFactor = valueToCode * codeZero;
            double scaleFactorChA = inputRangeChA_volts / (valueToCode * codeRange);
            double scaleFactorChB = inputRangeChB_volts / (valueToCode * codeRange);
            ...
            try
            {
                ...
                fixed (UInt16* pBuffer = byteToShortArray.ushorts)
                {
                    ...
                    for (record = 0; record < recordsPerCapture; record++)
                    {
                        for (int channel = 0; channel < channelCount; channel++)
                        {
                            ...
                            // 16-bit sample codes in each 16-bit sample value. 
                            //
                            ...
                            if (saveData)
                            {
                                ...
                                {
                                    channelName = 'A';
                                    scaleFactor = scaleFactorChA;
                                }
                                else
                                {
                                    channelName = 'B';
                                    scaleFactor = scaleFactorChB;
                                }
                                ...
                                UInt16* pSample = pBuffer;
                                for (UInt32 sample = 0; sample < samplesPerRecord; sample++)
                                {
                                    // Get sample value from buffer
                                    UInt16 sampleValue = *pSample++;

                                    // Convert 12-bit sample code in most significant bits 
                                    // of 16-bit sample value to volts
                                    double sampleVolts = scaleFactor * ((double) sampleValue - offsetFactor);
