...
namespace NPT_WaitNextBufferDiscard
{
    // This console program demonstrates how to configure an ATS9462-FIFO
    // to make a No-PreTrigger (NPT) AutoDMA acquisition. 
    ...
    class AcqToDiskApp
    {
        ...
        static public unsafe bool AcquireData(IntPtr boardHandle)
        {
            ...
            UInt32 recordsPerBuffer = 100;

            // TODO: Specifiy the total number of buffers to capture

            UInt32 buffersPerAcquisition = 100;
            ...
            try
            {
                ...
                fixed (short* pBuffer = byteToShortArray.shorts)
                {
                    ...
                    UInt32 admaFlags = AlazarAPI.ADMA_EXTERNAL_STARTCAPTURE |   // Start acquisition when we call AlazarStartCapture 
                                       AlazarAPI.ADMA_NPT |						// Acquire multiple records with no-pretrigger samples
                                       AlazarAPI.ADMA_FIFO_ONLY_STREAMING |		// The ATS9462-FIFO does not have on-board memory
                    ...
                    retCode =
                        AlazarAPI.AlazarBeforeAsyncRead(
                            ...
                            -(int)preTriggerSamples,	// S32 -- offset from trigger in samples
                    ...
                    while (!done)
                    {
                        // Find the number of full buffers 

                        Int32 buffersPendingFull;
                        retCode =
                            AlazarAPI.AlazarGetParameter(
                                boardHandle,        // HANDLE -- board handle
                                0,                  // U8 -- channel
                                AlazarAPI.GET_ASYNC_BUFFERS_PENDING_FULL,   // U32 -- parameter
                                &buffersPendingFull // S32* -- value
                                );
                        if (retCode != AlazarAPI.ApiSuccess)
                        {
                            throw new System.Exception("Error: AlazarGetParameter failed -- " +
                                AlazarAPI.AlazarErrorToText(retCode));
                        }

                        // Find the number of empty buffers 

                        Int32 buffersPendingEmpty;
                        retCode =
                            AlazarAPI.AlazarGetParameter(
                                boardHandle,        // HANDLE -- board handle
                                0,                  // U8 -- channel
                                AlazarAPI.GET_ASYNC_BUFFERS_PENDING_EMPTY,   // U32 -- parameter
                                &buffersPendingEmpty// S32* -- value
                                );
                        if (retCode != AlazarAPI.ApiSuccess)
                        {
                            throw new System.Exception("Error: AlazarGetParameter failed -- " +
                        ...
                        if (!discardBuffer)
                        {
                            // discard the buffer when the number of full buffers exceeds
                            // the number of empty buffers
                            discardBuffer = buffersPendingFull > buffersPendingEmpty;
                        }
                        else
                        {
                            // disacrd all but the last buffer filled
                            discardBuffer = buffersPendingFull > 1;
                        }
                        ...
                        if (discardBuffer)
                            retCode = AlazarAPI.AlazarWaitNextAsyncBufferComplete(boardHandle, null, 0, timeout_ms);
                        else
                        ...
                        if (!discardBuffer)
                        {
                            ...
                            {
                                for (UInt32 record = 0; record < recordsPerBuffer; record++)
                                {
                                    if (saveData)
                                    {
                                        // Write record to file
                                        fs.Write(buffer, byteOffset, (int)bytesPerRecord);
                                    }

                                    // Seek to next record in buffer
                                    byteOffset += (int)bytesPerRecord;
                                }
                            }
                        }
